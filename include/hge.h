...
enum hgeIntState
{
	...
	HGE_FXVOLUME		= 0x0405,   // int		global fx volume	(default: 100)
	HGE_MUSVOLUME		= 0x0406,   // int		global music volume	(default: 100)
	
	HGE_FPS				= 0x0407,	// int		fixed fps			(default: HGEFPS_UNLIMITED)

	//Add by Thor/h5nc
	//Begin
	HGE_FRAMECOUNTER	= 0x0408,	//int		frame counter;	read only
	HGE_FRAMESKIP		= 0x0409,	//int		frame skip			(default: 0)
	HGE_RENDERSKIP		= 0x040A,	//int		render skip			(default: 0)
	//End
...
public:
	...
	virtual void		CALL	Gfx_RenderText(HD3DFONT font, const char * text, float x, float y, float w, float h, DWORD color = 0xffffffff, bool shadow = true) = 0;
	// end

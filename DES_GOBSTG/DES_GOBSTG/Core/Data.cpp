...
DWORD Data::sLinkChara(DWORD sec, BYTE chara)
{
	switch(chara)
	{
	case M_CHARA_SHI_LINGYE:
		sec &= DATASBINMASK_CHARA;
		sec |= dataMap[DATATCHARA_A].binname;
		break;
	case M_CHARA_NUSIKUIMA_GEJI:
		sec &= DATASBINMASK_CHARA;
		sec |= dataMap[DATATCHARA_B].binname;
		break;
	case M_CHARA_CILAN_ZANGYUE:
		sec &= DATASBINMASK_CHARA;
		sec |= dataMap[DATATCHARA_C].binname;
		break;
	case M_CHARA_WUSHUANG:
		sec &= DATASBINMASK_CHARA;
		sec |= dataMap[DATATCHARA_D].binname;
		break;
	}
	return sec;
}
...
DWORD Data::sLinkDiff(DWORD sec, BYTE diff)
{
	...
	case M_DIFFI_PHANTASM:
		sec &= DATASBINMASK_DIFF;
		sec |= dataMap[DATATDIFF_P].binname;
		break;
...
int Data::getRangeNumber(int sno)
{
...
char * Data::getRangeName(int sno)
{
...
char * Data::getRangeUserName(int sno)
{
	strcpy(buf, DATA_DEFAULTSTR_WIDE);
	for(vector<rangeData>::iterator i=res.rdata.begin(); i!=res.rdata.end(); i++)
	{
...
char * Data::getRangeExplain(int sno, BYTE expi)
{
	strcpy(buf, DATA_DEFAULTSTR_WIDE);
	for(vector<rangeData>::iterator i=res.rdata.begin(); i!=res.rdata.end(); i++)
	{
		if(i->sno == sno)
		{
			...
			{
			case 1:
				strcpy(buf, i->explain_1);
				break;
			case 2:
				strcpy(buf, i->explain_2);
				break;
			}
			break;
		}
...
void Data::MoveDown(DWORD sec, BYTE i)
{
	...
	fWrite(DATA_BINFILE, secD, nLinkType(DATAN_CIRCLERATE), fRead(DATA_BINFILE, secS, nLinkType(DATAN_CIRCLERATE), 0));
	...
	iWrite(DATA_BINFILE, secD, nLinkType(DATAN_RANGEGET), iRead(DATA_BINFILE, secS, nLinkType(DATAN_RANGEGET), 0));
	...
	iWrite(DATA_BINFILE, secD, nLinkType(DATAN_ALIVENESS), iRead(DATA_BINFILE, secS, nLinkType(DATAN_ALIVENESS), 0));
...
void Data::getFile(BYTE type)
{
	...
	case DATA_RANGEACCESSFILE:
		filename = rangeaccessfilename;
		break;
	case DATA_RANGEDEFINEFILE:
		filename = rangedefinefilename;
		break;
...
{
	if(binmode && (type & DATA_TEXTHEADER) ||
...
#ifdef __DEBUG
	HGELOG("Data File %s rebuilt", filename);
#endif
	return true;
}
...
bool Data::SetFile(const char * _filename, BYTE type)
{
	...
	if(_access(filename, 00) == -1 ||
		!CheckHeader(type))
	{
		if(type == DATA_BINFILE || type == DATA_RANGEACCESSFILE)
		{
			if(!Init(type))
			{
				goto failed;
...
int Data::nMeet(int sno, bool bRange /* = false */, BYTE chara /* = 0xff */)
{
	if (chara >= M_NCHARA)
	{
		int ret = 0;
		...
		{
			ret += nMeet(sno, bRange, i);
		}
		return ret;
	}
	DWORD sec;
	sec = sLinkType(DATAS_RANGE);
	sec = sLinkChara(sec, chara);
...
int Data::nGet(int sno, bool bRange /* = false */, BYTE chara /* = 0xff */)
{
	if (chara >= M_NCHARA)
	{
		int ret = 0;
...
LONGLONG Data::nHighScore(int snostage, BYTE difflv, bool bRange /* = false */, bool bPractice /* = false */, BYTE chara /* = 0xff */)
{
	if (chara >= M_NCHARA)
	{
		LONGLONG ret = 0;
		for (int i=0; i<M_NCHARA; i++)
		{
			LONGLONG ths = nHighScore(snostage, difflv, bRange, bPractice, i);
			if (ths > ret)
			{
				ret = ths;
			}
...
int Data::nTryStageTime(int stage, BYTE difflv, BYTE chara /* = 0xff */)
{
	if (chara >= M_NCHARA)
	{
		int ret = 0;
		for (int i=0; i<M_NCHARA; i++)
		{
			ret += nTryStageTime(stage, difflv, i);
		}
		return ret;
	}
...
BYTE Data::getDiff(int sno)
{
	...
	if(sno >= S900 && sno < SA00)
		return 3;
	if(sno >= SA00 && sno < SB00)
		return 4;
	return 5;
}

...
bool BResource::Fill()
{
	...
	for (int i=0; i<SplitString(resdata.scriptfoldername); i++)
	{
		if(_access(scriptfolder[i], 00) == -1)
		{
			CreateDirectory(scriptfolder[i], NULL);
		}
	}
...
bool BResource::Pack(void * pStrdesc, void * pCustomConstName)
{
...
bool BResource::Gain(void * pStrdesc, void * pCustomConstName)
{
	BYTE * content;
	DWORD size;
	bool ret = false;

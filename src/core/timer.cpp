...
float CALL HGE_Impl::Timer_GetFPS(int mod)
{
	...
	else
	{
		...
		if(nFrameCounter % mod == 1)
		{
			if (nFrameCounter > fLastFrameCounter && fTime > fLastTime)
			{
				nowFPS = 1 / ((fTime - fLastTime) / (nFrameCounter - fLastFrameCounter));
			}
			fLastTime = fTime;
			fLastFrameCounter = nFrameCounter;
		}
		return nowFPS;
	}
}
